#!/bin/bash
#Requires .env vars: IP_ISSUER, IP_HOLDER, REGISTRY, CREDENTIAL_ID_HOLDER, IP_VERIFIER, IP_VERIFIER_DELIVERY
...
echo "Uploading Public Key of Issuer to Heimdall of Verifier Delivery to verify against it later, saving as issuer_pk.json"
curl -s --request POST "http://$IP_VERIFIER_DELIVERY/upload/file?name=issuer_pk.json" --form "uplfile=@issuer_pk.json"
