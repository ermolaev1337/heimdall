#!/bin/bash
#Requires .env vars: IP_HOLDER, IP_VERIFIER, PRESENTATION_EXPIRATION_SECONDS_2, PRESENTATION_CHALLENGE_2
...
echo "Uploading expiration and challenge to Heimdall of Verifier to verify against them later, saving as expiration_2.txt, and challenge_2.txt"
curl -s --request POST "http://$IP_VERIFIER/upload/file?name=expiration_2.txt" --form "uplfile=@expiration_2.txt"
curl -s --request POST "http://$IP_VERIFIER/upload/file?name=challenge_2.txt" --form "uplfile=@challenge_2.txt"
