#!/bin/bash
#Requires .env vars: IP_HOLDER, IP_VERIFIER, PRESENTATION_EXPIRATION_SECONDS_1, PRESENTATION_CHALLENGE_1
...
echo "Uploading expiration and challenge to Heimdall of Verifier to verify against them later, saving as expiration_1.txt, and challenge_1.txt"
curl -s --request POST "http://$IP_VERIFIER/upload/file?name=expiration_1.txt" --form "uplfile=@expiration_1.txt"
curl -s --request POST "http://$IP_VERIFIER/upload/file?name=challenge_1.txt" --form "uplfile=@challenge_1.txt"
