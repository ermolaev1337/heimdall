#!/bin/bash
#Requires .env vars: IP_HOLDER, CHALLENGE_2, IP_VERIFIER

echo "Generate Attribute Presentation for Holder, saving as pres_attribute_before_revocation.json"
curl -s "http://$IP_HOLDER/heimdalljs/pres/attribute?index=10&expiration=100&challenge=$CHALLENGE_2&secretKey=holder_sk.txt&destination=pres_attribute_before_revocation.json&credential=cred_holder.json" > pres_attribute_after_revocation.json
